...
$output = '';
try {
    $stmt = $con->prepare("SELECT * FROM program_comments WHERE progID = :program_id AND reply_id = 0 ORDER BY comments_date ASC");
    $stmt->execute([':program_id' => $_GET['programid']]);
    ...
    if (empty($result)) {
        ...
        foreach ($result as $row) {
            ...
            $stmt_replies = $con->prepare("SELECT * FROM program_comments WHERE reply_id = :comment_id");
